...
import { notifyAppSignalError } from "./appsignal_notify.tsx";
...
// Global error handler — catches all unhandled errors thrown in endpoints
// Each route sets c.set('errorMessage', '...') for a custom user-facing message
app.onError((error: Error, c: any) => {
  const userMessage = c.get('errorMessage') || `Failed to ${c.req.method} ${c.req.path}`;

  // Always log full error for debugging
  console.error(`[ERROR] ${userMessage}:`, error);

  // AppSignal Notification
  const tenantId = c.req.header('x-tenant-id');
  const routeForAppSignal = `[${c.req.method}] ${c.req.path}`;
  notifyAppSignalError(error, userMessage, routeForAppSignal, tenantId).catch(console.error);
...
app.get("/items", async (c) => {
  c.set('errorMessage', 'Failed to fetch inventory items');
...
app.get("/items/:id", async (c) => {
  c.set('errorMessage', 'Failed to fetch inventory item');
  const authHeader = c.req.header('Authorization');
  const tenantId = c.req.header('x-tenant-id');
...
app.post("/items", async (c) => {
  c.set('errorMessage', 'Failed to create inventory item');
  const authHeader = c.req.header('Authorization');
  const tenantId = c.req.header('x-tenant-id');
...
app.put("/items/:id", async (c) => {
  c.set('errorMessage', 'Failed to update inventory item');
  const authHeader = c.req.header('Authorization');
  const tenantId = c.req.header('x-tenant-id');
  const id = c.req.param("id");
  const body = await c.req.json();
...
app.delete("/items/:id", async (c) => {
  c.set('errorMessage', 'Failed to delete inventory item');
  const authHeader = c.req.header('Authorization');
  const tenantId = c.req.header('x-tenant-id');
  const id = c.req.param("id");
...
app.post("/distribute", async (c) => {
  c.set('errorMessage', 'Failed to distribute items');
  const authHeader = c.req.header('Authorization');
  const tenantId = c.req.header('x-tenant-id');
  const body = await c.req.json();
...
app.post("/stock-out", async (c) => {
  c.set('errorMessage', 'Failed to stock out items');
  const authHeader = c.req.header('Authorization');
  const tenantId = c.req.header('x-tenant-id');
  const body = await c.req.json();
...
app.post("/initialize", async (c) => {
  c.set('errorMessage', 'Failed to initialize database');
  const authHeader = c.req.header('Authorization');
  const tenantId = c.req.header('x-tenant-id');
...
app.get("/changelog", async (c) => {
  c.set('errorMessage', 'Failed to fetch changelog entries');
...
app.get("/changelog/export", async (c) => {
  c.set('errorMessage', 'Failed to export changelog');
...
app.delete("/changelog", async (c) => {
  c.set('errorMessage', 'Failed to clear changelog');
